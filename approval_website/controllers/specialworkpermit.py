...
class SpecialWorkPermitController(http.Controller):
    @http.route(['/special_work_permit/form'], type='http', auth="public", website=True)
    ...
    @http.route(['/special_work_permit/submit'], type='http', auth="public", website=True, csrf=False, methods=['POST'])
    def submit_special_work_permit(self, **post):
        try:
            ...
            return request.render("approval_website.special_work_permit_form", {
                'error': str(e),
                'values': post,
                'main_contractors': request.env['new.res.partner.company'].sudo().search([]),
                'sub_contractors': request.env['new.res.partner.company'].sudo().search([]),
