...
class EquipmentMaterialController(http.Controller):
    @http.route(['/equipment_material_2'], type='http', auth="public", website=True)
    def equipment_material_form(self, **kw):
        ...
        # 直接渲染表單頁面
        return request.render("approval_website.equipment_material_entry_form2", values)
    @http.route(['/equipment_material_2/submit'], type='http', auth="public", website=True, csrf=False, methods=['POST'])
    def submit_equipment_material(self, **post):
        try:
            ...
            return request.render("approval_website.equipment_material_entry_form2", {
                'error': str(e),
                'values': post,
                'main_contractors': request.env['new.res.partner.company'].sudo().search([]),
                'sub_contractors': request.env['new.res.partner.company'].sudo().search([]),
